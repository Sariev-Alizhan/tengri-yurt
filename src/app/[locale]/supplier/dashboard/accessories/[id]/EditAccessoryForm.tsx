...
export function EditAccessoryForm({
  ...
  const [photos, setPhotos] = useState<string[]>(accessory.photos || []);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`/api/supplier/accessories/${accessory.id}`, {
        method: 'PATCH',
      ...
      if (!res.ok) throw new Error(data.error || 'Failed to update accessory');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || 'Failed to delete accessory');
      }
      window.location.href = `/${locale}/supplier/dashboard/accessories`;
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full border border-white/30 bg-[#0f0d0a] text-white px-3 py-2 font-inter"
          >
        ...
        <div>
          <label className="flex items-center gap-2 font-inter text-white/80">
            <input
              name="is_available"
              type="checkbox"
              defaultChecked={accessory.is_available}
